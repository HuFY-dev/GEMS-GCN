...
def train(model, predictor, graph, split_edge, optimizer, batch_size):
    ...
    for perm in DataLoader(range(train_edges.size(0)), batch_size, shuffle=True):
        ...
        edge = torch.randint(0, graph.num_nodes(), edge.size(), dtype=torch.long, device=emb_x.device)
...
def main():
    ...
    optimizer = torch.optim.Adam(list(model.parameters()) + list(predictor.parameters()), lr=0.0005)
